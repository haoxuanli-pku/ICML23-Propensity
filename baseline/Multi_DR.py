...
def para(args): 
    if args.dataset == 'product': 
        ...
        args.imputation_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 1e-5}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 1e-5}
        ...
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 1e-6}
    elif args.dataset == 'coat':
        args.training_args = {'batch_size': 256, 'epochs': 500, 'patience': 20, 'block_batch': [64, 64]}
        args.base_model_args = {'emb_dim': 64, 'learning_rate': 0.05, 'weight_decay': 0.01}
        args.imputation_model_args = {'emb_dim': 64, 'learning_rate': 0.05, 'weight_decay': 0.01}
        args.propensity_model_args = {'emb_dim': 64, 'learning_rate': 0.05, 'weight_decay': 0.01}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        propensity_model_args: dict = {'emb_dim': 4, 'learning_rate': 0.005, 'weight_decay': 10}): 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                loss_all = sum_criterion(torch.sigmoid(y_hat_all), torch.sigmoid(g_all + y_hat_all_detach)) # sum(e_hat)
                y_hat_obs = base_model(users_train, items_train)
                y_hat_obs_detach = torch.detach(y_hat_obs)
                y_hat_obs = torch.sigmoid(y_hat_obs)
                g_obs = imputation_model(users_train, items_train)
                e_obs = none_criterion(y_hat_obs, y_train)
                e_hat_obs = none_criterion(y_hat_obs, torch.sigmoid(g_obs + y_hat_obs_detach))
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, unif_train, validation, test, device, base_model_args = args.base_model_args, imputation_model_args = args.imputation_model_args,training_args = args.training_args, propensity_model_args= args.propensity_model_args)
