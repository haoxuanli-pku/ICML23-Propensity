...
def para(args): 
    if args.dataset == 'product':
        ...
        args.base_model_args = {'emb_dim': 64, 'learning_rate': 1e-5, 'weight_decay': 1e-3}
        args.imputation_model_args = {'emb_dim': 64, 'learning_rate': 1e-5, 'weight_decay': 1}
        ...
        args.base_model_args = {'emb_dim': 16, 'learning_rate': 0.001, 'weight_decay': 1}
        args.imputation_model_args = {'emb_dim': 16, 'learning_rate': 0.001, 'weight_decay': 1}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
    ...
    base_optimizer = torch.optim.Adam(base_model.parameters(), lr=base_model_args['learning_rate'], weight_decay=0)

    imputation_model = MF(n_user, n_item, dim=imputation_model_args['emb_dim'], dropout=0).to(device)
    imputation_optimizer = torch.optim.Adam(imputation_model.parameters(), lr=imputation_model_args['learning_rate'], weight_decay=0)
