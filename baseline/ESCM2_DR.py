...
def para(args): 
    if args.dataset == 'product':
        ...
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 0.01, 'weight_decay': 1}
        args.imputation_model_args = {'emb_dim': 256, 'learning_rate': 0.01, 'weight_decay': 1}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 0.01, 'weight_decay': 1}
        args.loss_args = {"alpha": 1, "beta": 1, "imputation": 1}
    elif args.dataset == 'yahooR3': 
        args.training_args = {'batch_size': 4096, 'epochs': 500, 'patience': 10, 'block_batch': [2000, 200]}
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 5e-3, 'weight_decay': 1e-5}
        args.imputation_model_args = {'emb_dim': 256, 'learning_rate': 5e-3, 'weight_decay': 1e-7}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 5e-3, 'weight_decay': 1}
        args.loss_args = {"alpha": 1, "beta": 1, "imputation": 1}
    elif args.dataset == 'coat':
        args.training_args = {'batch_size': 512, 'epochs': 500, 'patience': 20, 'block_batch': [64, 64]}
        args.base_model_args = {'emb_dim': 64, 'learning_rate': 0.0005, 'weight_decay': 1e-4}
        args.imputation_model_args = {'emb_dim': 64, 'learning_rate': 0.0005, 'weight_decay': 1e-5}
        args.propensity_model_args = {'emb_dim': 64, 'learning_rate': 0.0005, 'weight_decay': 1}
        args.loss_args = {"alpha": 1, "beta": 1, "imputation": 1} 
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        loss_args: dict = {"alpha": 1, "beta": 1, "imputation": 1}
        ): 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                e_true = none_criterion(r_hat, sub_r) 
                ...
                loss_base = ctr_loss + loss_args["alpha"] * ctcvr_loss + loss_args["beta"] * cvr_loss_mnar + loss_args["imputation"] * loss_imp
       
                
                base_optimizer.zero_grad()
                imputation_model.zero_grad()
        ...
        print('Epoch: {0:2d} / {1}, Training: {2}, Validation: {3}'.
...
if __name__ == "__main__": 
    ...
    val_results, test_results = train_and_eval(train, unif_train, validation, test, device, training_args = args.training_args, base_model_args = args.base_model_args,imputation_model_args = args.imputation_model_args,   propensity_model_args = args.propensity_model_args, loss_args = args.loss_args)
