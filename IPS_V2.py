...
def para(args): 
    if args.dataset == 'product': 
        args.training_args = {'batch_size': 4096, 'epochs': 500, 'patience': 30, 'block_batch': [2000, 200]}
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 0}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 1e-3}
        args.loss_args = {"alpha": 1, "beta": 10, "gamma": 0.5}
    elif args.dataset == 'yahooR3': 
        args.training_args = {'batch_size': 4096, 'epochs': 500, 'patience': 30, 'block_batch': [2000, 200]}
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 0.005, 'weight_decay': 1e-6}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 0.01, 'weight_decay': 5e-4}
        args.loss_args = {"alpha": 1, "beta": 5, "gamma": 5e-2}
    elif args.dataset == 'coat':
        args.training_args = {'batch_size': 512, 'epochs': 500, 'patience':60, 'block_batch': [64, 64]}
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 1e-3, 'weight_decay': 1e-5}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 1e-2, 'weight_decay': 1e-1}
        args.loss_args = {"alpha": 1, "beta": 1, "gamma": 1e-2}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        base_model_args: dict = {'emb_dim': 256, 'learning_rate': 0.01, 'weight_decay': 0.1}, 
        propensity_model_args: dict = {'emb_dim': 256, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
        training_args: dict =  {'batch_size': 512, 'epochs': 100, 'patience': 20, 'block_batch': [3000, 300]},
        loss_args: dict = {"alpha": 1, "beta": 1, "gamma": 0.1}
        ): 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                cvr_loss_mnar = none_criterion(r_hat, sub_r)
                cvr_loss_mnar = torch.mean(torch.divide(torch.multiply(sub_observed, cvr_loss_mnar), p_hat))
                ones_all = torch.ones(len(p_hat)).to(device)
                w_all = torch.divide(sub_observed,p_hat)-torch.divide((ones_all-sub_observed),(ones_all-p_hat))
                bmse_loss = (torch.mean(w_all * r_hat))**2
                
                loss = ctr_loss + loss_args["alpha"] * ctcvr_loss + loss_args["beta"] * cvr_loss_mnar + loss_args["gamma"] * bmse_loss  
...
if __name__ == "__main__": 
    ...
    val_results, test_results = train_and_eval(train, unif_train, validation, test, device, training_args = args.training_args, base_model_args = args.base_model_args, propensity_model_args = args.propensity_model_args, loss_args = args.loss_args)
