...
def para(args): 
    if args.dataset == 'product':
        ...
        args.imputation_model_args = {'emb_dim': 256, 'learning_rate': 5e-4, 'weight_decay': 1e-6}
        args.propensity_model_args = {'emb_dim': 256, 'learning_rate': 5e-4, 'weight_decay': 1}
        args.loss_args = {"alpha": 1, "beta": 2,"gamma": 1e-1, "imputation": 1e-3} 
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        loss_args: dict = {"alpha": 1, "beta": 1, "rho": 1, "gamma": 1, "imputation": 1}
        ): 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                loss_base = ctr_loss + loss_args["alpha"] * ctcvr_loss + loss_args["beta"] * cvr_loss_mnar + loss_args["imputation"] * loss_imp + loss_args["gamma"] * bmse_loss
