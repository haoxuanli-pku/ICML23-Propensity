...
def seed_randomly_split(df, ratio, split_seed, shape):
    ...
    test = sp.csr_matrix((rating[idx[1]:], (rows[idx[1]:], cols[idx[1]:])),
                             shape=shape, dtype='float32')
    # print("random", sum(rating[idx[1]:]),len(rating[idx[1]:]))


    return train, valid, test

def load_dataset(data_name='coat', type = 'explicit', unif_ratio = 0.05, seed=0, implicit=True, threshold=4, device = 'cuda'): 
    ...
    unif_train, validation, test = seed_randomly_split(df=random_df, ratio=ratio, split_seed=seed, shape=(m, n))
    ...
    # print("train", sum(user_df['rating']),len((user_df['rating'])))
...
def load_dataset_rd(data_name='yahooR3', type = 'explicit', unif_ratio = 0.05, seed=0, implicit=True, threshold=4, device = 'cuda'): 
    if type not in ['explicit', 'implicit', 'list']: 
        print('--------illegal type, please reset legal type.---------')
        return
    path = os.path.dirname(os.path.dirname(__file__)) + '/datasets/' + data_name
